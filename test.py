...
if sys.version_info[0] == 2:
    def list_to_bytes(data): return ''.join([chr(x) for x in data])
    def bytes_to_list(data): return [ord(x) for x in data]
else:
...
class CryptSM4(object):
    ...
    def _f(cls, x0, x1, x2, x3, rk):
        ...
        def _sm4_l_t(ka):
            ...
            b[3] = SM4_BOXES_TABLE[a[3]]
            bb = get_uint32_be(b[0:4])
            c = bb ^ (rotl(bb, 2)) ^ (rotl(bb, 10)) ^ (
                rotl(bb, 18)) ^ (rotl(bb, 24))
            return c
